...
namespace WebResourceLinkerVsix
{
    public partial class CrmConnection : Form
    {
        ...
        private IOrganizationService BeginGetCrmConnection(string url, string org, string domain, string username, string password)
        {
            ...
            {
                string publicUrl = "";
                sdk = QuickConnection.Connect(url, domain, username, password, org, out publicUrl);
                this.PublicUrl = publicUrl;

                Controller.SaveConnectionDetails(this.LinkerDataPath, url, org, domain, username, password, publicUrl);
            }
            catch (Exception ex)
            {
